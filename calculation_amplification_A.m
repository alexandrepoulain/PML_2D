function [A] = calculation_amplification_A(dt,beta,gamma,bulk,mu,ng,TB,...
...
for ii = 1:size(TB,1)
    ...
    for k = 1:ng^2
        ...
        [Mat_FQ, Mat_Feps] = fun_F(a0,b0,x0,h,L2,n,cs,b_scal,dt,y);
...
% first line
A(1:nnodes*dof,:) = [dt^2*beta*M^-1*(C+Ceff) ...
    eye(size(M))+dt^2*beta*M^-1*(K+Keff) ...
    dt^2*beta*M^-1*Mat_epsi_n ...
    dt^2*beta*M^-1*Mat_Epsi_n ...
    dt^2*beta*M^-1*Mat_Sig_n];
A(nnodes*dof+1:2*nnodes*dof,:) = [eye(size(M))+dt*gamma*M^-1*(C+Ceff) ...
    dt*gamma*M^-1*(K+Keff) ...
    dt*gamma*M^-1*Mat_epsi_n ...
    dt*gamma*M^-1*Mat_Epsi_n ...
    dt*gamma*M^-1*Mat_Sig_n];
A(2*nnodes*dof+1:2*nnodes*dof+(3*nnodes),:) = [-Mat_Bepsi_n ...
    -Mat_BQ_n ...
    eye((ng^2)*3) ...
    zeros((ng^2)*3) ...
    zeros((ng^2)*3)];
A(2*nnodes*dof+(3*nnodes)+1:2*nnodes*dof+2*(3*nnodes),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    -dt*eye((ng^2)*3) ...
    eye((ng^2)*3) ...
    zeros((ng^2)*3)];
A(2*nnodes*dof+2*(3*nnodes)+1:2*nnodes*dof+3*(3*nnodes),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    -dt*DD*eye((ng^2)*3) ...
    zeros((ng^2)*3) ...
    eye((ng^2)*3)];
