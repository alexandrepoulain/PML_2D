...
% first line
B(1:nnodes*dof,:) = [-dt*eye(size(M))+dt^2*(0.5-beta)*M^-1*(C+Ceff) ...
    -eye(size(M))+dt^2*(0.5-beta)*M^-1*(K+Keff) ...
    dt^2*(0.5-beta)*M^-1*Mat_epsi_n ...
    dt^2*(0.5-beta)*M^-1*Mat_Epsi_n ...
    dt^2*(0.5-beta)*M^-1*Mat_Sig_n];
B(nnodes*dof+1:2*nnodes*dof,:) = [-eye(size(M))+dt*(1-gamma)*M^-1*(C+Ceff) ...
    dt*(1-gamma)*M^-1*(K+Keff) ...
    dt*(1-gamma)*M^-1*Mat_epsi_n ...
    dt*(1-gamma)*M^-1*Mat_Epsi_n ...
    dt*(1-gamma)*M^-1*Mat_Sig_n];
B(2*nnodes*dof+1:2*nnodes*dof+(3*nnodes),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    -Mat_Fepsi_n ...
    Mat_FQ_n ...
    zeros(size(Mat_Fepsi_n))];
B(2*nnodes*dof+(3*nnodes)+1:2*nnodes*dof+2*(3*nnodes),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_Fepsi_n)) ...
    -eye(size(Mat_Fepsi_n)) ...
    zeros(size(Mat_Fepsi_n))];
B(2*nnodes*dof+2*(3*nnodes)+1:2*nnodes*dof+3*(3*nnodes),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_Fepsi_n)) ...
    zeros(size(Mat_Fepsi_n)) ...
    -eye(size(Mat_Fepsi_n))];
