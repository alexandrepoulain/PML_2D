...
% first line
A(1:nnodes*dof,:) = [eye(size(M))+dt*gamma*M^-1*(C+Ceff) ...
    dt*gamma*M^-1*(K+Keff) ...
    dt*gamma*M^-1*Mat_epsi_n ...
    -dt*gamma*M^-1*Mat_Epsi_n ...
    dt*gamma*M^-1*Mat_Sig_n];
A(nnodes*dof+1:2*nnodes*dof,:) = [dt^2*beta*M^-1*(C+Ceff) ...
...
A(2*nnodes*dof+(3*ng^2)+1:2*nnodes*dof+2*(3*ng^2),:) = [zeros(size(Mat_BQ_n)) ...
    ...
    eye((ng^2)*3) ...
    zeros((ng^2)*3)];
A(2*nnodes*dof+2*(3*ng^2)+1:2*nnodes*dof+3*(3*ng^2),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    -dt*DD ...
    zeros((ng^2)*3) ...
    eye((ng^2)*3)];
