function [vect_past,P_past] = internal_forces_PML(dof,ng,Mat_Sig_n,vect_Sig_n,Mat_epsi_n,...
        vect_epsi_n,Mat_Epsi_n,vect_Epsi_n,P_past,T,G,Nef)
...
for i=1:Nef
    for s = 1:ne
        P_past((T(i,s)-1)*dof+1)=P_past((T(i,s)-1)*dof+1) + vect_past((i-1)*8+(s-1)*dof+1);
        P_past((T(i,s)-1)*dof+2)=P_past((T(i,s)-1)*dof+2) + vect_past((i-1)*8+(s-1)*dof+2);         
    end
end
