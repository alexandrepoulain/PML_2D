function [A] = Copy_of_calculation_amplification_A(dt,beta,gamma,bulk,mu,ng,TB,...
...
%         nnodes: nuber of nodes by element
%         dof: number of dof
...
for ii = 1:size(TB,1)
    ...
    for k = 1:ng^2
        ...
        J = tempdN*Xe;
        Mat_D = D/dt;
        [Mat_B, Mat_Bp, Mat_Be] = Copy_of_fun_B(a0,b0,L2,n,x0,h,cs,b_scal,Xe,eta,ksi,dt,k);
        ...
        Cell_Be{k}((ii-1)*8+1:(ii-1)*8+8,(ii-1)*3+1:(ii-1)*3+3) = abs(det(J))*w(k)*Mat_Be';
        Cell_Bp{k}((ii-1)*8+1:(ii-1)*8+8,(ii-1)*3+1:(ii-1)*3+3) = abs(det(J))*w(k)*Mat_Bp';
...
for kk = 1:ng^2
    Mat_Be_n(:,(kk-1)*3+1:(kk-1)*3+3)= Cell_Be{kk}(:,:);
    Mat_Bp_n(:,(kk-1)*3+1:(kk-1)*3+3)= Cell_Bp{kk}(:,:);
    ...
    DD((kk-1)*3+1:(kk-1)*3+3,(kk-1)*3+1:(kk-1)*3+3) = D; 
end

% first line
A(1:nnodes*dof,:) = [eye(size(M))+dt*gamma*M^-1*(C) ...
    dt*gamma*M^-1*(K) ...
    dt*gamma*M^-1*Mat_Be_n...
    dt*gamma*M^-1*Mat_Bp_n ...
    zeros(size(Mat_Bp_n))...
    zeros(size(Mat_Bp_n))];
A(nnodes*dof+1:2*nnodes*dof,:) = [dt^2*beta*M^-1*(C) ...
    eye(size(M))+dt^2*beta*M^-1*(K) ...
    dt^2*beta*M^-1*Mat_Be_n ...
    dt^2*beta*M^-1*Mat_Bp_n ...
    zeros(size(Mat_Bp_n))...
    zeros(size(Mat_Bp_n))];
A(2*nnodes*dof+1:2*nnodes*dof+(3*ng^2),:) = [zeros(size(Mat_Bepsi_n)) ...
    zeros(size(Mat_BQ_n)) ...
    eye((ng^2)*3) ...
    zeros((ng^2)*3) ...
    -DD...
    zeros((ng^2)*3)];
A(2*nnodes*dof+(3*ng^2)+1:2*nnodes*dof+2*(3*ng^2),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    -dt*eye((ng^2)*3) ...
    eye((ng^2)*3) ...
    zeros((ng^2)*3)...
    zeros((ng^2)*3)];
A(2*nnodes*dof+2*(3*ng^2)+1:2*nnodes*dof+3*(3*ng^2),:) = [-Mat_Bepsi_n ...
    -Mat_BQ_n ...
    zeros((ng^2)*3) ...
    zeros((ng^2)*3) ...
    eye((ng^2)*3)...
    zeros((ng^2)*3)];
A(2*nnodes*dof+3*(3*ng^2)+1:2*nnodes*dof+4*(3*ng^2),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    zeros((ng^2)*3) ...
    zeros((ng^2)*3) ...
    -dt*eye((ng^2)*3)...
