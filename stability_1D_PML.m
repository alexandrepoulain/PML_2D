...
S = 1;      % area of the bar
L = 1;    % length of PML
x0 = 0;   % start of the PML: end of phsical medium
...
for i=1:length(h)
    ...
    A = [eye(size(M^-1*K))+h(i)*(1-gamma)*M^-1*C gamma*h(i)*M^-1*K -h(i)*gamma*M^-1*Ap;
         ...
         zeros(size(M)) zeros(size(M)) eye(2)];
     B = -[-eye(size(M^-1*K))+ h(i)*(1-gamma)*M^-1*C (1-gamma)*h(i)*M^-1*K -h(i)*(1-gamma)*M^-1*Ap;
         ...
         zeros(2) -h(i)*tempB*alpha_inv -eye(2)+h(i)*cs/L*(w(1)*fp(pos1)/((1+fe(pos1))+cs/L*fp(pos1)*dt) + w(2)*fp(pos2)/((1+fe(pos2))+cs/L*fp(pos2)*dt))];
