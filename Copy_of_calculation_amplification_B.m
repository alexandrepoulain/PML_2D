...
% first line
B(1:nnodes*dof,:) = [-eye(size(M))+dt*(1-gamma)*M^-1*(C) ...
    dt*(1-gamma)*M^-1*(K) ...
    dt*(1-gamma)*M^-1*Mat_Be_n ...
    dt*(1-gamma)*M^-1*Mat_Bp_n ...
    zeros(size(Mat_Be_n))...
    zeros(size(Mat_Be_n))];
B(nnodes*dof+1:2*nnodes*dof,:) = [-dt*eye(size(M))+dt^2*(0.5-beta)*M^-1*(C) ...
    -eye(size(M))+dt^2*(0.5-beta)*M^-1*(K) ...
    dt^2*(0.5-beta)*M^-1*Mat_Be_n ...
    dt^2*(0.5-beta)*M^-1*Mat_Bp_n ...
    zeros(size(Mat_Be_n))...
    zeros(size(Mat_Be_n))];
B(2*nnodes*dof+1:2*nnodes*dof+(3*ng^2),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    zeros(3*ng^2) ...
    zeros(3*ng^2) ...
    zeros(3*ng^2)...
...
B(2*nnodes*dof+2*(3*ng^2)+1:2*nnodes*dof+3*(3*ng^2),:) = [zeros(size(Mat_BQ_n)) ...
    ...
    -Mat_Fepsi_n...
    Mat_FQ_n];
B(2*nnodes*dof+3*(3*ng^2)+1:2*nnodes*dof+4*(3*ng^2),:) = [zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_BQ_n)) ...
    zeros(size(Mat_Fepsi_n)) ...
    zeros(size(Mat_Fepsi_n)) ...
    zeros(size(Mat_Fepsi_n))...
